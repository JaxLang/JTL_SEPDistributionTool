...
\end{table}
    ...
    \textit{Attendees:} Jan Gieseler, Nina Dresing, Jax T Lang. \\

    \begin{enumerate}
        \item Start should create two folders: one for storing the raw data files (cdf's), and another `user output' folder that shows any plots or csv files they want.
        \item JTL to add more markdown descriptions to the user (as well as warnings about certain cells taking quite a while). And ``For more information, see the appendix below".
        \item Write a new df (as a deepcopy of previous steps df) for each new function: \texttt{df\_raw, df\_bg, df\_ic, \& df\_rs}.\\
        If the user wants to avoid performing certain functions then they need to respond to prompt as "FALSE" then the deep copy is used rather than adjusting the data.
        \item Use peak fits for the initial gauss parameters.
        \item When calling the peak fitting function, allow the user to provide the end time for the peaks to appear after the given start time. Default to 5 hours. 
        \item Use \texttt{plt.close("all")} to not plot the gaussian fits.
        \item Solarmach loop: 
        \begin{itemize}
            \item Ask the user if they want to search for the vsw each time or provide a default value (like 400 km/s).
            \item Must check if the user wants to update the previously loaded data (if any settings are changed like resample, sc, datetimes). \texttt{reuse\_previous\_data = True}.
        \end{itemize}
        \item Plot the intensity profiles before calculating the solarmach loop to allow the user to check the event before waiting too long.
        \item JG added \texttt{tqdm} function to py file to show progress bar.
        \item \textbf{Gauss fits need to use a reference point to calculate to avoid problems with Stonyhurst/Carrington coords limits.} Use the flare longitude as the reference point, else use the footpoint of the sc with the max(intensity) at the time of the max intensity. Then still show the correct coordinates on plots.\\
        \begin{itemize}
            \item e.g. flare at 65$^{\circ}$ shows 100 degree range as: 15 - 115$^{\circ}$.
            \item e.g. flare at 130$^{\circ}$ shows 100 degree range as: 80 - 180$^{\circ}$.
            \item e.g. flare at -160$^{\circ}$ shows 100 degree range as: (-210) - (-110)$^{\circ}$ \\
            OR using 360 degrees: flare at 200$^{\circ}$ with a 100 degree range: 150 - 250$^{\circ}$.
        \end{itemize}
        \item Need a name for the tool. Can't use SEPspread. Nina's looking at SEP Spatial Distribution Tool.
    \end{enumerate}

\textbf{Looking to do a main code \textit{handover} next week Monday, when Jan will try to incorporate it to the SOLER Tools github.}



    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \chapter{December 8, 2025}
    \textit{JTL thoughts: Naming conventions} \\

    The other names within the SERPENTINE/SOLER toolbox are:
    \begin{itemize}
        \item \textbf{Formal Title: \texttt{Python class name}: \textit{(notebook name)}}
        \item Solar-MACH: \\
        \texttt{from solarmach import SolarMACH}\\ 
        \textit{(solarmach.ipynb)}
        \item SEP Data Loader: \textit{(data\_loader.ipynb)}
        \begin{itemize}
            \item \texttt{from solo\_epd\_loader import epd\_load}
            \item \texttt{from seppy.loader.psp import psp\_isois\_load}
            \item \texttt{from seppy.loader.soho import soho\_load}
            \item \texttt{from seppy.loader.stereo import stereo\_load}
            \item \texttt{from seppy.loader.wind import wind3dp\_load}
        \end{itemize}
        \item Multi Spacecraft Plot:\\ 
        \texttt{from multi\_inst\_plots import Event}\\
        \textit{(SEP\_Multi-Spacecraft-Plot.ipynb)}
        \item Multi Instrument Plot:\\ 
        \texttt{import multi\_inst\_plots as m}\\ 
        \textit{(SEP\_Multi-Instrument-Plot.ipynb)}
        \item SEP Pitch-angle distribution (PAD) and first-order anisotropies:\\ 
        \texttt{from anisotropy import run\_SEPevent}\\ 
        \textit{(SEP\_PADs-and-Anisotropy.ipynb)}
        \item PyOnset: \\
        \texttt{import seppy.tools.widgets\_onset as w} \\
        \texttt{from pyonset import Onset, BootstrapWindow}\\ 
        \textit{(SEP\_PyOnset.ipynb)}
        \item SEP Regression Onset Tool: \\
        \texttt{import regression\_onset as reg}\\
        \texttt{from seppy.tools import Event}\\ 
        \textit{(SEP\_Regression-Onset.ipynb)}
        \item SEP Fluence Spectra Tool:\\ 
        \texttt{from fluence import Event}\\
        \textit{(SEP\_Fluence-Spectra.ipynb)}
        \item SEP Spectra Tool:\\ 
        \texttt{from spectra import Event}\\
        \textit{(SEP\_Spectra.ipynb)}
    \end{itemize}

    Based on Nina's comment in the last meeting, this tool would be called something along the lines of:\\
    \textbf{SEP Spatial Distribution Analysis} \textit{(SEP\_Spatial-Distribution.ipynb)}\\
    \texttt{from spatial\_dist\_analysis import SpatialEvent as sd\_event}
...
%	\include{multiToC}
    ...
    \subsection{Uncertianty calculation of the Gaussian result}

    When calculating the Gaussian fit, the following parameters are returned: $A$, $X_0$, and $\sigma$ (and their errors), and the residual.

    To calculate the uncertainty to plot the error range, we use:
    \begin{align}
        \Delta y_i &= y_i \; \ln(10) \sqrt{\sum (\beta_{errors} \; \cdot \tfrac{\partial y}{\partial \beta})^2 }
    \end{align}
    such that:
    \begin{align}
        y &= A \cdot \exp\left[-\frac{(x-X_0)^2}{2 \sigma^2} \right] \\
        \ln(y) &= \ln(A) - \frac{(x-X_0)^2}{2 \; \ln(10) \; \sigma^2}
    \end{align}
    gives us
    \begin{align}
        \Delta [\ln(y)] &= \sqrt{\sum \left(\Delta \beta \cdot \tfrac{\partial \ln(y)}{\partial \beta}\right)^2 } \\
        \tfrac{\partial \ln(y)}{\partial \ln(A)} &= 1\\
        \tfrac{\partial \ln(y)}{\partial X_0} &= \frac{x - X_0}{\ln(10) \; \sigma^2}\\
        \tfrac{\partial \ln(y)}{\partial \sigma} &=  \frac{(x-x_0)^2}{\ln(10) \; \sigma^3} \\
        \implies \Delta [\ln(y)] &= \sqrt{\left(\Delta \ln(A) \cdot 1\right)^2 + \left(\Delta X_0 \cdot  \frac{x - X_0}{\ln(10) \; \sigma^2} \right)^2 + \left( \Delta \sigma \cdot \frac{(x-x_0)^2}{\ln(10) \; \sigma^3} \right)^2} \\
        \implies \Delta y &= y \ln(10) \Delta [\ln(y)].
    \end{align}

...
\end{table}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % \chapter{December 1, 2025}
    % \textit{JTL thoughts.} \\

    % Ensuring the location of the flare is not a problem (i.e. if the points are [150, 175, -165, -130], then the curve fit won't work):
    % \begin{itemize}
    %     \item If the abs(longitude) for at least 3 sc > 90 then add 360 to the negative values. \textbf{This results in limits of 0:360.}
    %     \begin{table*}[h!]
    %         \centering
    %         \begin{tabular}{rcccc}
    %              original & 150 & 175 & -165 & -130 \\
    %              updated & 150 & 175 & 195 & 230 \\
    %         \end{tabular}
    %     \end{table*}
    %     \item If the abs(longitude) for at least 3 sc < 90, then leave it. \textbf{This results in limits of -180:180.}
    %     \begin{table*}[h!]
    %         \centering
    %         \begin{tabular}{rcccc}
    %              original & -150 & -100 & -45 & 30 \\
    %              updated & -150 & -100 & -45 & 30 \\
    %         \end{tabular}
    %     \end{table*}
    % \end{itemize}
    % Alternatively:
    % \begin{itemize}
    %     \item If the max amplitude comes from a sc with abs(longitude) > 90. Then the limits are 0:360.
    %     \item if the max amplitude comes from a sc with abs(longitude) < 90. Then the limits are -180:180.
    % \end{itemize}
    % or both...
    % \textit{Using the peak calculations: the sc with the max(intensity) has an abs(longitude) > 90 \textbf{and} the abs(longitude) of at least 3 sc is > 90; the limits become 0:360.}
